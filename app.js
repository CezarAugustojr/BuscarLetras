...
const insertNextAndPreviButtons = ({ prev, next }) => {
...
const insertSongsIntoPage = ({ data, prev, next }) => {
    ...
    if (prev || next) {
        insertNextAndPreviButtons({ prev, next })
        return
    }
...
const handleFormSubmit = event => {
    ...
    if (!searchTerm) {
        songsContainer.innerHTML = `<li class="warning-messege">Por Favor, digite um termo válido</li>`
        return
    }
...
const handSongsContainerClick = event => {
...
songsContainer.addEventListener('click', handSongsContainerClick)